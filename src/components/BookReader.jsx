...
import { BackIcon, BookmarkIcon, FileIcon, RestartIcon } from './Icons.jsx'
...
/**
 * The flipbook itself, plus the meta header and the bookmark counter.
 *
 * `book` is either a catalogue entry or an upload shaped like one; `source` is
 * whatever DearFlip should open — a URL under public/ for catalogue books, or a
 * blob: URL for a PDF the visitor picked from their own device. It arrives null
 * while an upload is still being read out of storage.
 */
export default function BookReader({ book, source, sourceError = null, backTo = '/books' }) {
  const slug = book.slug
  const holderRef = useRef(null)
  const flipbookRef = useRef(null)
  const [viewerError, setViewerError] = useState(null)
  const [page, setPage] = useState(() => getBookmark(slug))
  const [pages, setPages] = useState(() => getEntry(slug)?.pages ?? book.pages ?? null)
  const [resumedAt] = useState(() => getBookmark(slug))
  const error = sourceError ?? viewerError
  ...
  useEffect(() => {
    ...
    const openPage = getBookmark(slug)
    ...
    loadDearFlip()
      .then(($) => {
        ...
        flipbookRef.current = $(el).flipBook(source, {
          height: '100%',
          webgl: true,
      ...
      .catch((e) => {
        ...
        setViewerError('The flipbook viewer failed to load.')
      })
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      ...
      saveBookmark(slug, p, Number.isFinite(total) ? total : null)
    }, POLL_MS)
      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:flex-wrap lg:items-center lg:gap-5">
        ...
        <div className="flex min-w-0 items-center gap-4 lg:contents">
          ...
          {book.cover ? (
            ...
            <span className="grid h-20 w-14 shrink-0 place-items-center rounded-xl bg-sky-soft text-sky-deep shadow-card">
              <FileIcon size={20} />
            </span>
          )}
      ...
      <div className="df-container mt-6 rounded-3xl border border-line bg-sky-soft p-2 sm:p-5">
        {error ? (
          ...
          <div className="grid h-[60vh] place-items-center text-center">
            <p className="font-bold text-ink-soft">Opening your PDF…</p>
          </div>
        ) : (
